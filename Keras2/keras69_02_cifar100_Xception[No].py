...
#2. 모델구성
base_model = Xception(weights='imagenet', include_top=False, input_shape=(32, 32, 3))
...
# base_model = Xception(weights='imagenet', include_top=False, shape=(32, 32, 3))
...
start = time.time()
model.fit(x_train, y_train, epochs=1, validation_split=0.4, batch_size=128, callbacks=[es, reduce_lr])
end = time.time() - start
