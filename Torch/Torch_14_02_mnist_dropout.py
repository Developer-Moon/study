...
# train_dataset = MNIST(path, train=True, download=True, transform=transf) # 사이즈가 15가 된걸 tensor형태로 바꿔주겠다
...
x_train, y_train = train_dataset.data/255. , train_dataset.targets   # train_dataset/255. 스케일링 된 것 
x_test, y_test = test_dataset.data/255. , test_dataset.targets # train_dataset/255. 스케일링 된 것 
...
print(np.min(x_train.numpy()), np.max(x_train.numpy())) # 데이터가 텐서타입이라 X

x_train, x_test = x_train.view(-1, 28*28), x_test.view(-1, 28*28) # reshape view 동일
...
# epoch : 21, loss : 0.0362, acc : 0.989, val_loss : 0.1161, val_acc : 0.971