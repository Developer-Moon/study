from sklearn.model_selection import train_test_split, cross_val_score, cross_val_predict, KFold, StratifiedKFold, StratifiedKFold, GridSearchCV, RandomizedSearchCV
...
model = RandomizedSearchCV(SVC(), parameters, cv=kfold, verbose=1, refit=True, n_jobs=-1) # 그리드 서치에서 가져오는것 중에 랜덤하게 가져와서 사용
                                                                                          # 42개중 10개를 랜덤으로 빼서 사용 
#3. 컴파일, 훈련
import time
start = time.time()
