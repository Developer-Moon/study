...
# fit_generator대신 fit써도 개안타
# hist = model.fit(xy_train, epochs=10, steps_per_epoch=32,      # 통상적으로 전체데이터/batch = 160/5 = 32   fit_generator에는 batch가 없다 위의 generator에 정의 해놓음
#                     validation_data=xy_test, validation_steps=4)  # fit_generator 이걸 쓰면 안에 한번에 훈련가능
...
hist = model.fit(xy_train, epochs=10, steps_per_epoch=32,      # 통상적으로 전체데이터/batch = 160/5 = 32   fit_generator에는 batch가 없다 위의 generator에 정의 해놓음
                   validation_split=0.2)  # fit_generator 이걸 쓰면 안에 한번에 훈련가능
