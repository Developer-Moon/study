...
print(start_time)                                                 # start - 1656033369.6949103

model.compile(loss='mse', optimizer='adam')                                                                 
model.fit(x_train, y_train, epochs=50, batch_size=1, verbose=0)   # verbose = 0 훈련과정을 보여주지 않는다 -> 바로 결과가 나온다 
                                                                  # 사람에게 훈련과정을 보여주게 위해 딜레이를 걸어 시간이 걸린다 그 과정을 안하기 위해 쓴다
end_time = time.time() - start_time                               # end - 10.109211683273315   
print("걸린시간 : ", end_time)
