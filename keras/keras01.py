#1. 데이터
import numpy as np      """ numpy 사용하기 """      # unmpy(=Numerical Python, 파이썬 라이브러리) : 벡터 및 행렬 연산에 있어서 매우 편리한 기능을 제공
...
model.fit(x, y, epochs=1900)                       # 훈련(fit) 데이터 x와 y를 쓸꺼고 epochs=1900번을 훈련 시킨다  loss: 1.1842e-15 e는 맨앞 숫자 1앞에 0이 15개 있다(0.00000.....118422)
...
result = model.predict([4])   #결과  w 곱하기 4를 한 값이 result      $$학습할때와 평가할때의 데이터가 달라야 한다 아닐경우 과적합??이 걸린다 훈련용 데이터와 평가용 데이터를 나눈다$$
